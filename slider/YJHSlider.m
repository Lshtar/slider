...
@property (nonatomic, strong) UIColor *fillXColor;
@property (nonatomic, strong) UIColor *shadowFilColor;
@property (nonatomic, assign) CGFloat shadowFillOpacity;
...
- (void) doInit
{
    ...
    self.fillLayer.contents = (id)self.fillImage.CGImage;
...
- (void)doInit
{
    ...
    self.thumbLayer.fillImage = _thumbImage;
...
- (void)drawRect:(CGRect)rect
{
    ...
    //绘主刻度轴(X轴)
    [self.scaleLineXColor setStroke];
    ...
    if (YJHSliderStyle_Nomal == self.sliderStyle || YJHSliderStyle_Cross == self.sliderStyle) {
        //绘制竖直刻度短线
        ...
        if (YJHSliderStyle_Nomal ==self.sliderStyle) {
            startY = MAX(0, spindleScaleStartPoint.y-self.scaleLineHeight);
        }else if (YJHSliderStyle_Cross == self.sliderStyle) {
    ...
    }else if (YJHSliderStyle_Point == self.sliderStyle) {
        //绘制圆点型刻度分隔
        NSInteger lineNum = self.scaleLineNumber+1;
        CGFloat oneW = (W-self.thumbDiameter)/self.scaleLineNumber;
        CGFloat x = self.thumbDiameter * 0.5;
...
- (void)setThumbImage:(UIImage *)thumbImage
{
    ...
    self.thumbLayer.fillImage = thumbImage;
}
